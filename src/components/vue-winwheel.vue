	...
	export default {
		...
		data() {
			return {
				...
				functionEvent: (event)=> {
					...
					if (event.keyCode === 13) {
						console.log(this.wheelSpinning !== undefined)
						if(!this.wheelSpinning && this.wheelSpinning !== undefined) 
							this.startSpin();
					}
				},
				WinWheelOptions: {
		...
		methods: {
			getWheelSize() {
				...
				this.size = window.outerHeight - 120
			},
			hidePrize() {
				this.modalPrize = {}
				this.showModal = true
				...
				if (this.wheelSpinning === false) {
					...
					this.theWheel = new Winwheel.Winwheel({
						...
						animation: {
							...
							spins: Math.floor(Math.random() * 20 + 1),
		...
		mounted() {
			...
			window.addEventListener('resize', () => this.getWheelSize())
		},
		destroyed() {
